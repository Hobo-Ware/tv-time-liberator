...
import {
    ...
    mr_nobody_watched,
    the_triangle_unwatched,
} from "../../.test/data";

describe("myLists", () => {
    it("should return my lists", async () => {
        const username = assertDefined(
            import.meta.env.TV_TIME_TEST_USERNAME,
            "TV_TIME_TEST_USERNAME not defined.",
        );
        const password = assertDefined(
            import.meta.env.TV_TIME_TEST_PASSWORD,
            "TV_TIME_TEST_PASSWORD not defined.",
        );
        ...
        const mr_nobody = watched.movies.find((item) =>
            item.uuid === mr_nobody_watched.uuid
        );
        ...
        const chernobyl = watched.shows.find((item) =>
            item.uuid === chernobyl_up_to_date.uuid
        );
        ...
        const alien = unwatched.movies.find((item) =>
            item.uuid === alien_unwatched.uuid
        );
        ...
        const the_triangle = unwatched.shows.find((item) =>
            item.uuid === the_triangle_unwatched.uuid
        );
