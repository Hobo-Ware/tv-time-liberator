...
import { USERNAME_HASH } from './env/TV_TIME_USERNAME';

const db = await PersistentStore.create(PersistentStore.namespacedPath(USERNAME_HASH));

async function fetchFlutterToken(): Promise<string> {
    const isFlutterTokenAvailable = await db.has(TvTimeValue.FlutterToken);
    if (!isFlutterTokenAvailable) {
        const token = await getToken();
        await db.set(TvTimeValue.FlutterToken, token);
        return token;
    }

    const cachedToken = await db.get<string>(TvTimeValue.FlutterToken);

    const invalidateAndRetry = async () => {
        await db.delete(TvTimeValue.FlutterToken);
...
async function fetchUser(): Promise<UserToken | null | undefined> {
    const isUserTokenAvailable = await db.has(TvTimeValue.UserToken);

    if (!isUserTokenAvailable) {
        return null;
    }

    const user = await db.get<UserToken>(TvTimeValue.UserToken);
    ...
    try {
        ...
        await db.delete(TvTimeValue.UserToken);
    }
}
...
export async function login(username: string, password: string): Promise<UserToken> {
    ...
    try {
        ...
        await db.set(TvTimeValue.UserToken, userToken);
